...
export default function StudentProfileSection() {
  ...
  const handleFaqSave = async (data: { title: string; content: string }) => {
    try {
      await api.post('/api/tutors/me/faqs', data);
      showToast('FAQ가 성공적으로 등록되었습니다.', 'success');
      setFaqOpen(false);
    } catch (error) {
      showToast(error?.message || 'FAQ 등록에 실패했습니다.', 'error');
    }
  };
  ...
  return (
    <div className="mb-4">
      ...
      {faqOpen && <FaqRegisterModal onSave={handleFaqSave} onClose={() => setFaqOpen(false)} />}
    </div>
  );
}
